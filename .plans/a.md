...
| D-5 | `SignUpForm.tsx` called `signInWithPassword()` immediately after `signUp()` | **B — ALREADY PATCHED BY A, NEEDS B'S REVIEW** | ⚠️ **Lane A edited three Lane B files**: `src/app/(auth)/sign-up/SignUpForm.tsx`, `src/i18n/en.ts`, `src/i18n/ar.ts`. Done at the owner's instruction because signup was broken and BE-4 is a MUST. Email confirmation stays ON — it is a security control. The form now shows "check your email" instead of signing in before the address is confirmed, and maps Auth's opaque "Database error saving new user" to a sentence about the civil ID. **If B has this file open, take B's version and re-apply the two changes** — they are small and the reasoning is in the comments. |
