...
11. **STRETCH `/prescriptions/drafts/[id]`** — each extracted field beside the image with its confidence, correct, accept into `prescriptions`. Serves D26. Proof: a seeded draft accepted appears in `/prescriptions` as `extracted`. Day: Wed 23, **only if the lead confirms on Tuesday**. Needs: Lane C extraction writing `prescription_drafts`; the fork. `status: TODO (stretch)`
